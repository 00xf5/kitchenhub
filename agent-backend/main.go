	...
	case "mouse_move":
		if x, ok := data["x"].(float64); ok {
			if y, ok := data["y"].(float64); ok {
				...
				// TODO: Pipe to control.exe: echo "mouse_move %d %d"
			}
		}
	case "mouse_down":
		if button, ok := data["button"].(string); ok {
			log.Printf("[INPUT] Mouse down: %s\n", button)
			// TODO: Pipe to control.exe: echo "mouse_down %s"
		}
	case "mouse_up":
		if button, ok := data["button"].(string); ok {
...
func handleVideoStream(w http.ResponseWriter, r *http.Request) {
	...
	// Goroutine 1: Send screenshots continuously
	...
	go func() {
		...
		for {
			...
			case <-stopCh:
				return
			case <-ticker.C:
				screenshot, err := os.ReadFile(session.screenshotPath)
				if err != nil {
					continue
				}
